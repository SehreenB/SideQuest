...
class DiscoveryEngine: ObservableObject {
    ...
    func fetchNearbyPlaces(
        ...
        let geminiStops = await geminiNearbyPlaces(location: location, mode: mode, travel: travel, desiredCount: 12)
        ...
        if APIKeys.googleMaps != "YOUR_GOOGLE_MAPS_API_KEY" {
            do {
                let stops = try await googleNearbyPlaces(location: location, mode: mode, travel: travel)
    ...
    private func googleNearbyPlaces(
        ...
        switch travel {
        case .walking: searchRadius = 2_000
        case .driving: searchRadius = 15_000
        }
        ...
        async let strictMode: [GoogleMapsService.PlaceResult] = {
            (try? await GoogleMapsService.shared.searchNearby(
                location: location,
                radius: max(1_500, min(searchRadius, 6_000)),
                type: type,
                keyword: keyword
            )) ?? []
        }()
