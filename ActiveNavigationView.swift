...
struct ActiveNavigationView: View {
    ...
    @State private var hasAnnouncedRouteStart = false
    @State private var fallbackSpeechSynth = AVSpeechSynthesizer()
    ...
    private var routeStartIndicator: GoogleMapView.RouteStartIndicator? {
        let path = visiblePolyline
        guard let first = path.first else { return nil }
        let bearing: CLLocationDirection
        if path.count > 1 {
            bearing = bearingDegrees(from: first, to: path[1])
        } else {
            bearing = userBearing ?? 0
        }
        return .init(coordinate: first, bearingDegrees: bearing)
    }
    ...
    var body: some View {
        ZStack {
            ...
            GoogleMapView(
                ...
                routeStartIndicator: routeStartIndicator
            )
            .ignoresSafeArea()
        ...
        .onAppear {
            ...
            loc.startUpdates()
            Task {
                await prepareRouteGeometry()
                try? await Task.sleep(nanoseconds: 600_000_000)
                await announceRouteStartIfNeeded()
            }
        }
        ...
        .onDisappear {
            ...
            fallbackSpeechSynth.stopSpeaking(at: .immediate)
        }
    ...
    private func exitRouteEarly() {
        // Exiting early should keep check-in points already earned,
        // but should not grant route-completion points.
        loc.stopUpdates()
        ElevenLabsService.shared.stopPlayback()
        fallbackSpeechSynth.stopSpeaking(at: .immediate)
        dismiss()
    ...
    private func announceRouteStartIfNeeded() async {
        guard voiceGuidanceEnabled else { return }
        guard !hasAnnouncedRouteStart else { return }
        guard !routeCompleted else { return }
        guard !ElevenLabsService.shared.isPlaying else { return }

        hasAnnouncedRouteStart = true

        if let firstStop = route.stops.first {
            let intro = "Navigation started. First stop: \(firstStop.name). \(firstStop.shortDescription)"
            await speakInstruction(intro)
            return
        }

        if let instruction = currentInstruction {
            await speakInstruction("Navigation started. \(instruction.text)")
        }
    }
    ...
    private func speakInstruction(_ text: String) async {
        ...
        isPlayingAudio = true
        do {
            try await playGuideWithTimeout(text: text, timeoutNanoseconds: 2_500_000_000)
        } catch {
            speakFallback(text)
            isPlayingAudio = false
        }
    ...
    private func playGuideWithTimeout(text: String, timeoutNanoseconds: UInt64) async throws {
    ...
    private func speakFallback(_ text: String) {
        ...
        utterance.rate = 0.49
        fallbackSpeechSynth.speak(utterance)
    }
}
