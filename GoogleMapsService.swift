...
final class GoogleMapsService {
    ...
    func placeDetails(placeID: String) async throws -> PlaceDetailsResult? {
        ...
        components.queryItems = [
            ...
            URLQueryItem(name: "fields", value: "place_id,name,formatted_address,geometry,opening_hours,photos,formatted_phone_number,international_phone_number,types"),
            URLQueryItem(name: "key", value: apiKey)
        ]
        ...
        return PlaceDetailsResult(
            ...
            formattedPhoneNumber: result["formatted_phone_number"] as? String,
            internationalPhoneNumber: result["international_phone_number"] as? String,
            types: result["types"] as? [String] ?? []
        )
    }

    /// Fetches editorial summary and a photo for a place using Places API v1.
    /// Falls back to Place Details API when editorial summary is unavailable.
    func placeEditorialDetails(placeID: String) async throws -> PlaceEditorialDetails {
        let descriptionFallback = "No description available."

        // Prefer Places API v1 for editorial summary.
        if let v1URL = URL(string: "https://places.googleapis.com/v1/places/\(placeID)") {
            var request = URLRequest(url: v1URL)
            request.httpMethod = "GET"
            request.setValue(apiKey, forHTTPHeaderField: "X-Goog-Api-Key")
            request.setValue("photos.name,editorialSummary", forHTTPHeaderField: "X-Goog-FieldMask")
            ...
            if let (data, response) = try? await URLSession.shared.data(for: request),
               ...
               let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                ...
                let photoName = ((json["photos"] as? [[String: Any]])?.first)?["name"] as? String

                let description = (editorialSummary?.isEmpty == false) ? editorialSummary! : descriptionFallback
                let photoURL = photoName.flatMap { placePhotoMediaURL(photoName: $0) }
                return PlaceEditorialDetails(description: description, photoURL: photoURL)
            }
        }

        // Fallback: legacy details endpoint (address + photo reference).
        if let legacy = try await placeDetails(placeID: placeID) {
            let fallbackDescription = legacy.formattedAddress.isEmpty ? descriptionFallback : legacy.formattedAddress
            let fallbackPhotoURL = legacy.photoReferences.first.flatMap { placePhotoURL(photoReference: $0, maxWidth: 800) }
            return PlaceEditorialDetails(description: fallbackDescription, photoURL: fallbackPhotoURL)
        }

        return PlaceEditorialDetails(description: descriptionFallback, photoURL: nil)
    }
    ...
    func searchNearby(
        ...
        return results.compactMap { result in
            ...
            return PlaceResult(
                ...
                photos: (result["photos"] as? [[String: Any]] ?? []).compactMap { $0["photo_reference"] as? String }
            )
        }
    }

    // Convenience alias for callers that use the requested helper name.
    func photoURL(for photoReference: String, maxWidth: Int = 400) -> URL? {
        placePhotoURL(photoReference: photoReference, maxWidth: maxWidth)
    }
    ...
    /// Places API v1 media URL for photo names like: places/{place_id}/photos/{photo_id}
    func placePhotoMediaURL(photoName: String, maxHeightPx: Int = 900) -> URL? {
        URL(string: "https://places.googleapis.com/v1/\(photoName)/media?maxHeightPx=\(maxHeightPx)&key=\(apiKey)")
    }
